...
// Helper XML IDÊNTICO AO POSTMAN
...
async function emitirCoris(leadData) {
    ...
    // Parâmetros de Emissão (InsereVoucherIndividualV13)
    const insereParams = {
        ...
        // Dados Pax 1 (Padrão individual ou primeiro do familiar)
        ...
        // Contato
        'contatonome': { val: leadData.contactName || pax1.nome.split(' ')[0], type: 'varchar' },
    ...
    if (erro && erro !== '0' && erro !== 'OK') {
        throw new Error(`Coris Emissão Falhou: ${extractTagValue(text, 'mensagem') || 'Erro desconhecido'} (Cód: ${erro})`);
    }
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!msResponse.ok) {
             ...
             throw new Error(`Pagamento Recusado pelo Gateway: ${errText}`);
        }
        ...
        if (body && body.leadId) await supabaseClient.from('remessaonlinesioux_leads').update({ status: 'pagamento_falhou', last_error_message: error.message }).eq('id', body.leadId);
